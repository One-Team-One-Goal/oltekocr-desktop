...
type EditableSessionMode = "OCR_EXTRACT" | "TABLE_EXTRACT";
...
export function PresetManagerDialog({
  ...
  const [mode, setMode] = useState<EditableSessionMode>("TABLE_EXTRACT");
  ...
  useEffect(() => {
    ...
    const nextMode: EditableSessionMode =
  ...
  const updateColumn = (
    ...
    setColumns((prev) => {
      const next = [...prev];
      const previousColumn = next[index];
      const updatedColumn = { ...previousColumn, [field]: value };

      if (field === "label") {
        const previousAutoKey = toKey(previousColumn.label);
        const keyWasAutoGenerated =
          !previousColumn.key || previousColumn.key === previousAutoKey;

        if (keyWasAutoGenerated) {
          updatedColumn.key = toKey(value);
        }
      }

      next[index] = updatedColumn;
      return next;
    });
  };
  ...
  return (
    <Dialog open={open} onOpenChange={(next) => !next && onClose()}>
      <DialogContent
        ...
        className="grid-rows-[auto_minmax(0,1fr)_auto] sm:max-w-[540px] h-[85vh] sm:h-[640px] overflow-hidden p-0 gap-0"
      >
        ...
        <div
          className={`grid ${presetsCollapsed ? "grid-cols-[44px_1fr]" : "grid-cols-[220px_1fr]"} flex-1 min-h-0`}
        >
          <div className="border-r p-2 overflow-y-auto">
            <div className="flex items-center justify-between gap-1 mb-2">
              {!presetsCollapsed && (
                <p className="text-[11px] font-medium text-muted-foreground px-1">Presets</p>
              )}
              <Button
                variant="ghost"
                size="icon"
